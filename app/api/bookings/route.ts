...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({reference:stored.reference,status:stored.status,service:stored.serviceSlug,requestedStart:stored.requestedStart,requestedEnd:stored.requestedEnd},{status:201,headers:{'Cache-Control':'no-store'}});
  }catch(error){
