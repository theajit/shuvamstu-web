...
const services=['astrology','online-puja','puja-rituals','bratopanayan','marriage'];
...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    else if(body.action==='provider-save'){const id=body.id?text(body.id,100):undefined;const name=text(body.name,100);const type=body.type;const zone=timezone(body.timezone);if(!name||!zone||(type!=='PANDIT'&&type!=='ASTROLOGER'))throw new Error('VALIDATION');await saveProvider({id:id||undefined,name,type,timezone:zone,active:body.active!==false})}
