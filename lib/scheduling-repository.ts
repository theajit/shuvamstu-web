...
export type StoredBooking = {reference:string;status:'REQUESTED'|'CONFIRMED';serviceSlug:string;requestedStart:string;requestedEnd:string};
...
class PostgresSchedulingRepository implements SchedulingRepository {
  ...
  async createBookingAtomically(input:AtomicBookingInput){
    return this.sql.begin(async transaction=>{
      ...
      for(let attempt=0;attempt<5;attempt+=1){const reference=generateBookingReference();try{const [stored]=await transaction<StoredBooking[]>`insert into bookings (reference,provider_id,service_slug,customer_name,customer_email,customer_phone,location_mode,venue,timezone,local_date,local_time,requested_start,requested_end,occupied_start,occupied_end,status,customer_message,capacity_used,buffer_before_minutes,buffer_after_minutes) values (${reference},${input.providerId},${input.serviceSlug},${input.customerName},${input.customerEmail},${input.customerPhone||null},${input.locationMode},${input.venue||null},${input.timezone},${input.localDate},${input.localTime},${input.requestedStart},${input.requestedEnd},${occupiedStart},${occupiedEnd},${status},${input.customerMessage||null},1,${configuration.bufferBeforeMinutes},${configuration.bufferAfterMinutes}) returning reference,status,service_slug as "serviceSlug",requested_start::text as "requestedStart",requested_end::text as "requestedEnd"`;return stored}catch(error){if(!(error instanceof postgres.PostgresError)||error.code!=='23505'||attempt===4)throw error}}
