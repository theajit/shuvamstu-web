...
export async function listAdminPujas():Promise<AdminPuja[]>{if(!sql)throw new Error('DATABASE_UNAVAILABLE');return sql<AdminPuja[]>`select p.id,p.slug,p.name,p.short_description as "shortDescription",p.full_description as "fullDescription",p.duration_minutes as "durationMinutes",case when min(pkg.service_amount_paise) is null then null else min(pkg.service_amount_paise)::numeric/100 end::float8 as "startingPriceRupees",p.samagri_required as "samagriRequired",p.samagri_available as "samagriAvailable",p.online_available as "onlineAvailable",p.temple_available as "templeAvailable",coalesce(p.notes,'') as notes,p.active,p.updated_at::text as "updatedAt",case when count(distinct tr.locale)=2 and bool_and(tr.status='COMPLETED') then 'COMPLETED' when bool_or(tr.status='FAILED') then 'FAILED' else 'PENDING' end as "translationStatus" from pujas p left join puja_packages pkg on pkg.puja_id=p.id and pkg.active left join puja_translations tr on tr.puja_id=p.id where p.realm='LIVE' and p.deleted_at is null group by p.id order by p.active desc,p.name`}
